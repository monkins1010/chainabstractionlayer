...
function testSignPSBTSimple(chain: Chain) {
    it('should sign psbt a simple send', async () => {
        ...
        const tx2: Transaction<VerusTypes.Transaction> = await fundAddress(chain, unusedAddressTwo.address, new BigNumber(1000000));
        ...
        const signedPSBTHex = await (chain.client.wallet as VerusBaseWalletProvider).signPSBT(psbt.toBase64(), [
        ...
        const payment3: Transaction<VerusTypes.Transaction> = await chain.client.chain.getTransactionByHash(payment3TxHash);
...
function testSignPSBTScript(chain: Chain) {
    ...
    it('should send p2sh and sign PSBT to redeem', async () => {
        ...
        for (const newAddress of newAddresses) {
            const address = await (client.wallet as VerusBaseWalletProvider).getWalletAddress(newAddress);
        ...
        const paymentVariant = bitcoinJs.payments.p2wsh({
            redeem: { output: multisigOutput, network: network as VerusTypes.VerusNetwork },
            network: network as VerusTypes.VerusNetwork,
        });

        const address = paymentVariant.address;

        const initiationTx: Transaction<VerusTypes.Transaction> = await client.wallet.sendTransaction({
            to: address,
            value,
        });
        ...
        let multiVout: VerusTypes.Output;
        ...
        const psbt = new bitcoinJs.Psbt({ network: network as VerusTypes.VerusNetwork });
        ...
        const signedPSBTHex = await (client.wallet as VerusBaseWalletProvider).signPSBT(psbt.toBase64(), [
            { index: 0, derivationPath: unusedAddressOneDerivationPath },
        ]);
        const signedPSBT = bitcoinJs.Psbt.fromBase64(signedPSBTHex, { network: network as VerusTypes.VerusNetwork });
        ...
        const claimTx = decodeRawTransaction(claimTxRaw, network as VerusTypes.VerusNetwork);
...
function testSignBatchP2SHTransaction(chain: Chain) {
    ...
    it("Should redeem two P2SH's", async () => {
        ...
        const addresses = [];
        for (const newAddress of newAddresses) {
            const address = await (chain.client.wallet as VerusBaseWalletProvider).getWalletAddress(newAddress);
            addresses.push(address);
        }
        ...
        const paymentVariantOne = bitcoinJs.payments.p2wsh({
            redeem: { output: multisigOutputOne, network: network as VerusTypes.VerusNetwork },
            network: network as VerusTypes.VerusNetwork,
        });
        const paymentVariantTwo = bitcoinJs.payments.p2wsh({
            redeem: { output: multisigOutputTwo, network: network as VerusTypes.VerusNetwork },
            network: network as VerusTypes.VerusNetwork,
        });
        ...
        const txb = new bitcoinJs.TransactionBuilder(network as VerusTypes.VerusNetwork);
        const txfee = VerusUtils.calculateFee(3, 3, 9);
        ...
        const signaturesOne = await (chain.client.wallet as VerusBaseWalletProvider).signBatchP2SHTransaction(
            [
                {
        ...
        const signaturesTwo = await (chain.client.wallet as VerusBaseWalletProvider).signBatchP2SHTransaction(
            [
                {
                    inputTxHex: initiationTx._raw.hex,
                    index: 0,
