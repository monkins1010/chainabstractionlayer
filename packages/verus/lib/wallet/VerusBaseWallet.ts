...
export abstract class VerusBaseWalletProvider<T extends VerusBaseChainProvider = any, S = any> extends Wallet<T, S> {
    ...
    protected async _getUsedUnusedAddresses(numAddressPerCall = 100, addressType: AddressSearchType) {
        ...
        while (
            ...
            const transactionCounts: AddressDeltas = await this.chainProvider.getProvider().getAddressTransactionCounts(addrList);

            for (const address of addrList) {
                const isUsed = transactionCounts[address.toString()].length > 0;
